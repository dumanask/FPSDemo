...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void Update()
    {
        if (photonView.IsMine)
        {
            ...
            if (invertlook)
            {
                viewPoint.rotation = Quaternion.Euler(verticalRotStore, viewPoint.rotation.eulerAngles.y, viewPoint.rotation.z);
            }
            else
            {
                viewPoint.rotation = Quaternion.Euler(-verticalRotStore, viewPoint.rotation.eulerAngles.y, viewPoint.rotation.z);
            ...
            if (!overHeated)
            {
                if (Input.GetMouseButtonDown(0) && allGuns[selectedGun].isAutomatic)
                {
                    Shoot();
                }
    ...
    private void Shoot()
    {
        ...
        //Instantiate(bullet, ray.origin, cam.transform.rotation);
